...
const { ensureMigrationTable, getAllFilesInFolder, replace, sleep } = require('../utils');
...
const DB = class {
    ...
    async migration(upOrDown) {
        ...
        try {
            ...
            await ensureMigrationTable({ query });
            ...
            const appliedMigrationNames = (await query({ sql: 'SELECT name FROM migration' })).map(m => m.name);
